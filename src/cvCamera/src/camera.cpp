...
//输出参数：img42_final（三通道RGB图像）
        ...
        for (size_t i = 0; i < (int)circles.size(); i++) {
            ...
            if((width + x) > srcImg.cols)
                width = srcImg.cols - x;
            if((height + y) > srcImg.rows)
                height = srcImg.rows - y;

            Rect temRect = Rect(x, y, width, height);

            boundRect.push_back(temRect);
            rectangle(img42, Point(temRect.x, temRect.y), Point(temRect.x + temRect.width, temRect.y + temRect.height),Scalar(0, 0, 255), 2, 8);
        ...
        for(int i = 0; i < (int)boundRect.size();i++) {
            ...
            for (int ii = 0; ii < (int) roiContours.size(); ii++) {
                ...
                if (maxLoc.x == 4 && maxVal * 100 > 60) {
                    // cout << "识别结果：" << maxLoc.x << "  相似度:" << maxVal * 100 << "%" << endl;
                    flag4 = true;

                }
                if (maxLoc.x == 2 && maxVal * 100 > 60) {
                    flag2 = true;
                    // cout << "识别结果：" << maxLoc.x << "  相似度:" << maxVal * 100 << "%" << endl;
                }
            ...
            if (flag4 || flag2) {
                ...
                /*if(fpsFlag >= 10)
                {
                    for(int tagIndex = 0;tagIndex < 9;tagIndex++)
                    {
                        tagPoint[tagIndex] = tagPoint[tagIndex + 1];
                    }

                    tagPoint[9].x = boundRect[i].x + boundRect[i].width / 2;
                    tagPoint[9].y = boundRect[i].y + boundRect[i].height / 2;
                }

                else
                {*/
                tagPoint[fpsFlag].x = boundRect[i].x + boundRect[i].width / 2;
                tagPoint[fpsFlag].y = boundRect[i].y + boundRect[i].height / 2;
                // }
                fpsFlag++;
                flag4 = false;
                flag2 = false;
        ...
        if(fpsFlag == fps)
        {
            ...
            if(sum / fps < disThr)
            {
                Point p(tagPoint[fps/2].x,tagPoint[fps/2].y);
                circle(img42_final, p, 10, Scalar(0, 0, 255));
                //send info
                move.camera_info = p.x - img42_final.cols/2;
